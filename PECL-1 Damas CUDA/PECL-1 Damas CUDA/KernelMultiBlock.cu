...
__global__ void DamasBomPlayMultiBlock(long *Tab, int numthread, int row, int col, int direcion) {
	int Col = blockIdx.x * TAM_TESELA + threadIdx.y;	// Identificador de hilo del eje Y, lo usamos para calular la columna.
...
__device__ void yellowBomMultyBlock(long *Tab, int x, int y, int width) { // Si me da tiempo ago que rompa mas cosas ademas de acer la traspuesta
	if (isBomtrasposeMultiBlock) {
		// Pos si le añdo mas cosas a esta bomba.
	}
}
	...
	for (size_t i = 0; i < ((x > gridDim.x) ? 2 : 3); i++) {
		...
		for (size_t j = 0; j < 2; j++) {
			...
			if ((victimas - (victimas % 10)) != (fichaInMov - (fichaInMov % 10))) {
				Tab[((Col + y + row) + i) * width + (Row + x + (new int[2]{ 1, -1 })[j])] = POS_TAB_JUEGO_EMPTY;
			}
		}
	}
